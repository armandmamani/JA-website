...
document.addEventListener("DOMContentLoaded", function() {
  ...
  document.querySelectorAll(".apLabel").forEach(label => {
    label.addEventListener("click", function(event) {
      ...
      if (linkedCheckbox) {
        ...
        linkedCheckbox.dataset.wasChecked = linkedCheckbox.checked
          ? "true"
          : "false";
        showPlan(apartmentId);
        selectApartment(apartmentId);
  ...
  document.getElementById("printButton").addEventListener("click", function() {
    const elementsToHide = document.querySelectorAll(
      "header, video, nav, .sidenav,.sidenav_r, .controls, .footer,#fullScreenPlanBtn,#sideImage, .sidebar,#shiturShkrimi, #rezervuarShkrimi, #fullscreenContainer, #SectorA-image, #printButton"
  ...
  function checkLinkedRadio(apartmentId) {
    ...
    const linkedRadio = document.querySelector(
      `.radioteDhena[value='${apartmentId}']`
    );
    if (linkedRadio) {
  ...
  function selectApartment(apartmentId) {
    ...
    if (localStorage.getItem("loggedIn") == "true") {
      const apartment = apartmentDetails.find(
        apartment => apartment.id === apartmentId
      );
  ...
  function updateLabelAndButtonStyles() {
    ...
    document.querySelectorAll(".radioteDhena").forEach(checkbox => {
      const apartment = apartmentDetails.find(
        apartment => apartment.id === checkbox.value
      );
      const label = document.querySelector(`label[for='${checkbox.id}']`);
      const button = document.querySelector(
        `.ApBtn[id='${checkbox.value}btn']`
      );
      if (localStorage.getItem("loggedIn") == "true") {
        if (apartment) {
          if (apartment.statusi === "Shitur") {
            ...
            if (label) {
              ...
              label.style.color = "#500e0e";
            }
            if (button) {
              button.style.backgroundColor = "#f4c95d";
              button.style.color = "#500e0e";
            }
  ...
  function updateForm() {
    ...
    selectedCheckboxes.forEach(checkbox => {
      ...
      if (apartment) {
        ...
        TotalVerandArea +=
          apartment.verandaArea +
            apartment.greenTerraceArea +
            apartment.usableTerrace || 0;
    ...
    if (tabelaVlerave) {
      ...
      document.getElementById("sVerandeEDhene").value = formatNumber(
        TotalVerandArea
      );
      ...
      const cmimParking =
        scaleFactor * parseFloat(document.getElementById("cmimParking").value);
      const cmimPishine =
        scaleFactor * parseFloat(document.getElementById("cmimPishine").value);

      const parkingCost = parkingCount * cmimParking;
      const poolCost = poolCount * cmimPishine;

      const cmimsNeto =
        scaleFactor * parseFloat(document.getElementById("cmimsNeto").value);
      const cmimsPerb =
        scaleFactor * parseFloat(document.getElementById("cmimsPerb").value);
      const cmimsVerande =
        scaleFactor * parseFloat(document.getElementById("cmimsVerande").value);
      const cmimsOborr =
        scaleFactor * parseFloat(document.getElementById("cmimsOborr").value);
      const cmimsDepo =
        scaleFactor * parseFloat(document.getElementById("cmimsDepo").value);
      ...
      const total =
        sTotale + sVerande + sOborr + sStoreArea + parkingCost + poolCost;
  ...
  function showForm(apartmentId) {
    ...
    if (selectedRadio) {
      selectedRadio.dataset.wasChecked = selectedRadio.checked
        ? "true"
        : "false";
...
document
  .querySelectorAll('input[type="checkbox"].radioteDhena')
  .forEach(checkbox => {
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  let imageBtn;
  if (ImageAbtn) imageBtn = ImageAbtn;
  ...
  document
    .getElementById("fullScreenPlanBtn")
    .addEventListener("click", function() {
      // Select the image inside the currently visible apartment plan
      const selectedPlanImage = document.querySelector(
        ".apartament-plan:not(.hidden) img"
      );
